...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 w-full z-[100] px-6 md:px-12 py-4 md:py-6 flex justify-between items-center bg-[#0a0c1b]/90 backdrop-blur-xl border-b border-white/5">
      ...
      {/* MENU MOBILE OVERLAY - BACKGROUND SOLID ABU-ABU */}
      <div className={`fixed inset-0 bg-[#111827] flex flex-col items-start justify-start pt-28 px-8 transition-all duration-300 z-[105] ${isOpen ? 'opacity-100 visible translate-x-0' : 'opacity-0 invisible translate-x-full'}`}>
          ...
          <div className="flex flex-col gap-6 w-full mt-4">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href} 
                className="text-2xl font-bold text-white hover:text-indigo-400 border-b border-white/10 pb-4"
          ...
          <div className="mt-10 w-full flex flex-col gap-6">
            <div className="flex items-center gap-8">
                <a href="https://linkedin.com/in/rafdi-zul-qisthi-a617a8360/" target="_blank" className="text-slate-400 hover:text-white transform scale-150">
