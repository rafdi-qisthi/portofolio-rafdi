...
<div className={`fixed inset-0 bg-[#0a0c1b] transition-all duration-300 z-[105] ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
    <div className="flex flex-col h-screen w-full px-8 pt-28 pb-12">
        
        {/* 1. LIST MENU UTAMA (Tengah) */}
        ...
        {/* 2. MENU TAMBAHAN (Bawah) agar persis seperti Desktop */}
        <div className="flex flex-col items-center gap-6 w-full mt-auto">
            ...
            <div className="flex items-center gap-12">
                ...
                <a href="/cv-rafdi.pdf" target="_blank" className="text-slate-300 hover:text-white font-bold text-xl border-b-2 border-indigo-500/50 pb-1">
                    Download CV
                </a>
            </div>

            {/* Tombol Kontak Full Width */}
            <a 
                href="#contact" 
                onClick={() => setIsOpen(false)} 
                className="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold text-center text-lg shadow-lg shadow-indigo-500/20 active:bg-indigo-700 transition"
            >
                Hubungi Saya (Kontak)
            </a>
        </div>
