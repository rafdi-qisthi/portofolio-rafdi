...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 w-full z-[100] px-6 md:px-12 py-4 md:py-6 flex justify-between items-center bg-[#0a0c1b]/90 backdrop-blur-xl border-b border-white/5">
      ...
      {/* TOMBOL HAMBURGER */}
      <button className="md:hidden text-white p-2 z-[110]" onClick={() => setIsOpen(!isOpen)}>
      ...
      {/* MENU MOBILE OVERLAY (SOLID BACKGROUND) */}
      <div className={`fixed inset-0 bg-[#0a0c1b] flex flex-col items-center justify-center transition-all duration-300 z-[105] ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
          <div className="flex flex-col items-center gap-8">
            {navLinks.map((link) => (
              <a 
                key={link.name} 
                href={link.href} 
                className="text-2xl font-semibold text-slate-200 hover:text-indigo-400 transition"
            ...
            {/* Menu Tambahan agar Lengkap seperti Desktop */}
            <div className="mt-4 flex flex-col items-center gap-6 pt-8 border-t border-white/10 w-64">
                <a href="/cv-rafdi.pdf" target="_blank" className="text-indigo-400 font-bold text-xl underline underline-offset-8">Download CV</a>
